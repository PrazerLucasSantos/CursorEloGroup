#!/usr/bin/env node
/**
 * Restaura o Dossiê da Entrega de Valor (Fase 1) no épico reorganizado.
 * - Tela única, cabeçalho fixo + 5 abas (DTIC, UGEPV, ASCOM, Monitoramento, Timeline).
 * - Travas de progressão por status, linhas embutidas, método "Devolver" e presets.
 * - Cria um novo domínio "07 · Entrega de Valor" e renumera Backlog/Servidor.
 *
 * Uso: node scripts/restore-dossie-entrega-valor.mjs
 */
...
// Grupos (domínio próprio para a entrega de valor pós-venda)
const GRP_MAIN = 'grp-07-entrega-valor'
const GRP_EMB = 'grp-07-entrega-valor-emb'
const GRP_MET = 'grp-07-entrega-valor-met'
...
function buildTransicaoForm() {
  return {
    id: FORM_TR,
    name: '(19.4) EV · Transição de status',
    sectionLayout: 'none',
...
function buildMarcoForm() {
  return {
    id: FORM_MR,
    name: '(19.5) EV · Marco da timeline',
    sectionLayout: 'none',
...
function buildDevolverForm() {
  return {
    id: FORM_DEVOLVER,
    name: '(19.M1) EV · Devolver entrega',
    sectionLayout: 'none',
...
function buildMainForm() {
  return {
    id: FORM_MAIN,
    name: '(19) Dossiê da Entrega de Valor',
    sectionLayout: 'tabs',
...
function patchGroups(groups) {
  // 1) renumera Backlog/Servidor para abrir espaço ao domínio 07
  for (const g of groups.groups) {
    if (g.id === 'grp-07-backlog') g.name = '08 · Backlog'
    if (g.id === 'grp-08-servidor') g.name = '09 · Servidor (backend)'
  }

  // 2) cria o domínio 07 · Entrega de Valor + subgrupos (idempotente)
  const ensureGroup = (def) => {
    if (!groups.groups.some((g) => g.id === def.id)) groups.groups.push(def)
  }
  ensureGroup({ id: GRP_MAIN, name: '07 · Entrega de Valor' })
  ensureGroup({ id: GRP_EMB, name: 'Embutidas', parentGroupId: GRP_MAIN })
  ensureGroup({ id: GRP_MET, name: 'Métodos', parentGroupId: GRP_MAIN })

  // 3) atribuições
  const embedded = [FORM_KPI, FORM_MED, FORM_PA, FORM_TR, FORM_MR]
  groups.assignments[FORM_MAIN] = GRP_MAIN
  for (const id of embedded) groups.assignments[id] = GRP_EMB
  groups.assignments[FORM_DEVOLVER] = GRP_MET

  // 4) ordem por grupo
  const setOrder = (gid, ids) => {
    groups.memberOrderByGroup[gid] = ids
  }
  setOrder(GRP_MAIN, [FORM_MAIN])
  setOrder(GRP_EMB, embedded)
  setOrder(GRP_MET, [FORM_DEVOLVER])
...
function patchWorkspace(ws) {
  const w = ws[0]
  const pkgId = 'pkg-mapa-5-entrega-valor'
  let pkg = w.packages.find((p) => p.id === pkgId)
  if (!pkg) {
    pkg = { id: pkgId, name: '5 · Entrega de Valor (19)', classes: [] }
    w.packages.push(pkg)
  }
  if (!pkg.classes.some((c) => c.id === 'cls-mapa-ev')) {
    pkg.classes.push({
      id: 'cls-mapa-ev',
      name: '(19) Dossiê da Entrega de Valor',
...
function main() {
  ...
  console.log('OK: Dossie da Entrega de Valor restaurado.')
  console.log('  Grupo: 07 - Entrega de Valor (+ Embutidas + Metodos)')
  console.log('  Forms: principal + 5 embutidas + metodo devolver')
}
