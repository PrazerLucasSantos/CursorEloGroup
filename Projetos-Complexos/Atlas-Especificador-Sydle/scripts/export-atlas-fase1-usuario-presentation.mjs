#!/usr/bin/env node
/**
 * Gera presentation.json da apresentação «Atlas — Fase 1: Da proposta ao cadastro do contrato»
 * (versão de USUÁRIO de negócio) e, com --zip, monta o ZIP estático autocontido.
 *
 * Uso:
 *   node scripts/sync-atlas-fase1-flows.mjs
 *   node scripts/export-atlas-fase1-usuario-presentation.mjs
 *   node scripts/export-atlas-fase1-usuario-presentation.mjs --zip
 */
...
import { flowFase1Usuario } from './atlas-fase1-usuario-flow.mjs'
...
const flowId = flowFase1Usuario.id
const baseName = 'fase-1-usuario'
...
if (wantZip) {
  ...
  console.log('Publique o ZIP na Netlify (arrastar) para obter link público.')
}
